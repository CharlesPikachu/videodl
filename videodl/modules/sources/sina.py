...
class SinaVideoClient(BaseVideoClient):
    ...
    def __init__(self, **kwargs):
        ...
        self.default_parse_headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36', 'Referer': 'https://video.sina.cn/'}
        self.default_download_headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36', 'Referer': 'https://video.sina.cn/'}
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            # --parse video id
            if not (video_id := pattern.match(url).group('id')):
                if pattern.match(url).group('token') is not None:
                    (resp := self.get(url, **request_overrides)).raise_for_status(); assert resp.url != url
                    return self.parsefromurl(resp.url, request_overrides=request_overrides)
                else:
            ...
            # --parse raw data
            video_data, formats = raw_data['data'], []; assert isinstance(video_data, dict)
            for quality_id, quality in (safeextractfromdict(video_data, ['videos', 'mp4'], {}) or {}).items():
                if not isinstance(quality, dict): continue
                if not (file_api := quality.get('file_api')) or not (file_id := quality.get('file_id')): continue
                formats.append({'quality': quality_id, 'download_url': f'{file_api}?foo=bar&vid={file_id}'})
            ...
            resp = self.get((download_url := formats[0]['download_url']), stream=True, **request_overrides)
            if not resp or resp.status_code > 400:
                ...
                raw_data['play'] = resp2json(resp=resp); formats = raw_data['play']['data']['videos']
            ...
            video_info.update(dict(title=video_title, save_path=os.path.join(self.work_dir, self.source, f'{video_title}.mp4'), ext='mp4', identifier=video_id, cover_url=safeextractfromdict(raw_data, ['data', 'image'], None)))
