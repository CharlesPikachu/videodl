...
class CCtalkVideoClient(BaseVideoClient):
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None) -> list[VideoInfo]:
        ...
        try:
            # --fetch video ids
            parsed_url = urlparse(url); sid = parse_qs(parsed_url.query, keep_blank_values=True).get('sid', [None])[0]
            ...
            for _, video_id in enumerate(video_ids):
                ...
                download_url = safeextractfromdict((raw_data := resp2json(resp=resp)), ['data', 'videoUrl'], '')
                if not download_url or not download_url.startswith('http'): continue
                (video_info_page := copy.deepcopy(video_info)).update(dict(raw_data=raw_data)); video_info_page.update(dict(download_url=download_url))
