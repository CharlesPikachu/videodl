...
class WeiboVideoClient(BaseVideoClient):
    ...
    def _parsefromurlwithmweibo(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            sorted_download_urls = sorted(download_urls.items(), key=lambda kv: get_resolution_score_func(kv[1]), reverse=True)
            ...
            video_title = legalizestring(raw_data["data"]['page_info'].get('title') or raw_data["data"]['page_info'].get('page_title') or null_backup_title, replace_null_string=null_backup_title).removesuffix('.')
