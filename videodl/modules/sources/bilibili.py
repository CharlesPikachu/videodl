    ...
    def _parsefromcommonurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            for page_idx, page in enumerate(raw_data['data']["pages"]):
                ...
                video_title = f"ep{page_idx}-{page.get('part')}" if len(raw_data['data']["pages"]) > 1 else raw_data["data"].get('title')
                ...
                video_page_info.update(dict(title=video_title, file_path=os.path.join(self.work_dir, self.source, f'{video_title}.{ext}'), ext=ext, guess_video_ext_result=guess_video_ext_result, identifier=f'{video_id}_{cid}'))
