...
class BilibiliVideoClient(BaseVideoClient):
    ...
    def _parsefromcommonurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            part_id = int(part_id[0]) if part_id and isinstance(part_id, list) and str(part_id[0]).lstrip("+-").isdigit() else None
            ...
            for page_idx, page in enumerate(raw_data['data']["pages"]):
                if (part_id and page_idx + 1 != part_id) or (not isinstance(page, dict)): continue
                try: (resp := self.get(f"https://api.bilibili.com/x/player/playurl?otype=json&fnver=0&fnval=0&qn=80&bvid={video_id}&cid={page['cid']}&platform=html5", **request_overrides)).raise_for_status()
                ...
                video_page_info.update(dict(download_url=(download_url := max(page_raw_data['data']['durl'], key=lambda x: x['size'])['url'])))
                video_title = f"ep{len(video_infos)+1}-{page.get('part')}" if len(raw_data['data']["pages"]) > 1 else safeextractfromdict(raw_data, ['data', 'title'], None)
    ...
    def _parsefrombangumiepurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            for item in safeextractfromdict(raw_data, ['result', 'section'], []): result_episodes += dict(item).get('episodes', [])
            for _, page in enumerate(result_episodes):
                if (not isinstance(page, dict)) or (str(page['ep_id']) != episode_id): continue
                ...
                video_page_info, formats = copy.deepcopy(video_info), []; video_page_info.update(dict(raw_data=page_raw_data))
    ...
    def _parsefrombangumissurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            for _, page in enumerate(result_episodes):
                try: (resp := self.get(f"https://api.bilibili.com/pgc/player/web/v2/playurl?fnval=12240&ep_id={page['id']}", **request_overrides)).raise_for_status()
                except Exception: continue
    ...
    def _parsefromcheeseepurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            for _, page in enumerate(raw_data['data']['episodes']):
                if (not isinstance(page, dict)) or (str(page['id']) != episode_id): continue
    ...
    def _getredirecturl(self, url: str, aid: str, request_overrides: dict = None):
        try: (resp := self.get("https://api.bilibili.com/x/web-interface/view", params={"aid": aid}, **request_overrides)).raise_for_status(); return self.get(resp2json(resp=resp)['data']['redirect_url'], allow_redirects=True, **request_overrides).url
