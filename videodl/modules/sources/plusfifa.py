...
class PlusFIFAVideoClient(BaseVideoClient):
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None):
        ...
        # try parse
        try:
            try: catalog_id = parse_qs(urlparse(url).query, keep_blank_values=True)['catalogId'][0]; video_id = re.search(r"/player/([^/?]*)", urlparse(url).path.strip('/')).group(1)
            except Exception: catalog_id = urlparse(url).path.strip('/').split("/")[-1]; (resp := self.get(PlusFIFAVideoClient.ASSET_URL.format(catalog_id=catalog_id), headers={'x-chili-device-id': self.DEVICE_ID}, **request_overrides)).raise_for_status(); assets = resp2json(resp=resp); video_id = next(asset["id"] for asset in assets if (isinstance(asset, dict) and str(asset.get("type", "")).lower() == "main"))
            try: (resp := self.get(PlusFIFAVideoClient.CONTENTS_URL.format(catalog_id=catalog_id).split("/child")[0], headers={'x-chili-device-id': self.DEVICE_ID}, **request_overrides)).raise_for_status(); raw_data['CONTENTS_URL_RESPONSE'] = resp2json(resp=resp); video_title = legalizestring(raw_data['CONTENTS_URL_RESPONSE']['title'] or null_backup_title, replace_null_string=null_backup_title).removesuffix('.')
            except Exception: m = re.search(r"/content/(.*?)/", urlparse(url).path.strip('/')); video_title = legalizestring((m.group(1) if m else video_id) or null_backup_title, replace_null_string=null_backup_title).removesuffix('.')
            (resp := self.post(PlusFIFAVideoClient.SESSION_URL, json={'videoAssetId': video_id}, headers={'x-chili-device-id': self.DEVICE_ID, 'x-chili-avod-compatibility': 'free,free-ads', 'x-chili-accept-stream': 'mpd/cenc+h264;mpd/clear+h264;mp4/', 'x-chili-accept-stream-mode': 'multi/codec-compatibility;mono/strict', 'x-chili-manifest-properties': 'subtitles'}, **request_overrides)).raise_for_status()
            raw_data['SESSION_URL_RESPONSE'] = resp2json(resp=resp); session_id = raw_data['SESSION_URL_RESPONSE']["id"]
            (resp := self.get(PlusFIFAVideoClient.STREAMING_URL, headers={'x-chili-streaming-session': session_id}, **request_overrides)).raise_for_status()
