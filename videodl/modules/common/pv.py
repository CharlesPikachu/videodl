...
class PVVideoClient(BaseVideoClient):
    ...
    def __init__(self, **kwargs):
        ...
        self.default_parse_headers = {"user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36", "content-type": "application/x-www-form-urlencoded; charset=UTF-8"}
    ...
    def _encrypturl(self, url_data: str):
        ...
        sig_bytes = 32; key_bytes = self._wordstokey(key_words, sig_bytes); data_bytes = url_data.encode("utf-8")
        block_size = 16; padding_length = block_size - (len(data_bytes) % block_size); padded_data = data_bytes + bytes([padding_length] * padding_length)
        cipher = AES.new(key_bytes, AES.MODE_ECB); encrypted = cipher.encrypt(padded_data)
    ...
    def _wordstohex(self, word_array):
        words = word_array["words"]; sig_bytes = word_array["sigBytes"]; byte_array = bytearray()
        for word in words: byte_array.extend(struct.pack(">i", word))
        result_bytes = bytes(byte_array[:sig_bytes]); hex_string = result_bytes.hex()
        return hex_string
    '''_encodeurl'''
    ...
    def _decryptresponse(self, encrypted_hex, key_hex):
        key = bytes.fromhex(key_hex); encrypted_bytes = bytes.fromhex(encrypted_hex)
        cipher = AES.new(key, AES.MODE_ECB); decrypted_bytes = unpad(cipher.decrypt(encrypted_bytes), AES.block_size); decrypted_text = decrypted_bytes.decode('utf-8')
    ...
    def _splitandsortformats(self, formats: List[Dict[str, Any]], include_muxed_in_audio: bool = False) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        nonecodec_func = lambda x: x is None or x in ("none", "")
        vcodecrank_func = lambda v, s=None: (lambda s: 3 if ("av01" in s or "av1" in s) else (2 if "vp9" in s else (1 if ("avc" in s or "h264" in s) else 0)))((v or "").lower())
        acodecrank_func = lambda a: (lambda s: 2 if "opus" in s else (1 if ("mp4a" in s or "aac" in s) else 0))((a or "").lower())
        label_func = lambda f: (f.get("format") or f.get("format_note") or f.get("quality") or "") or ""
        parsefromlabel_func = lambda label: ((lambda s: (lambda up: (lambda h: (lambda fps: (h, fps, int(any(k in s for k in ("高码率", "高比特", "高帧率"))), int(any(k in up for k in ("HDR", "DOLBY", "DV")) or ("杜比" in s) or ("真彩" in s))))(int(m.group(1)) if (m := re.search(r'(\d{2,3})\s*(?:fps|FPS|帧)\b', s)) else (int(m.group(1)) if h > 0 and (m := re.search(r'[pP]\s*(\d{2,3})\b', s)) and 24 <= int(m.group(1)) <= 240 else 0)))(int(m.group(1)) if (m := re.search(r'(\d{3,4})\s*[pP]\b', s)) else (4320 if "8K" in up else 2160 if "4K" in up else 1440 if "2K" in up else 0)))(s.upper()))(label.strip()))
        infervideofields_func = lambda f: ((lambda h, w, fps, high_br=0, hdr_dolby=0: (lambda nh, nfps, nbr, nhdr: (nh, W_BY_H[nh] if (w == 0 and nh in W_BY_H) else w, nfps, nbr, nhdr))(*((lambda ph, pfps, pbr, pdh: (ph if h == 0 else h, pfps if fps == 0 else fps, pbr, pdh))(*parsefromlabel_func(label_func(f))) if ((h == 0 or fps == 0) and (not nonecodec_func(f.get("vcodec")) or label_func(f))) else (h, fps, high_br, hdr_dolby))))(f.get("height") or 0, f.get("width") or 0, f.get("fps") or 0))
        lookslikevideo_func = lambda f: (True if not nonecodec_func(f.get("vcodec")) else (lambda h, lab: (h > 0) or ("HDR" in lab) or ("DOLBY" in lab) or ("DV" in lab) or ("4K" in lab) or ("8K" in lab))(infervideofields_func(f)[0], label_func(f).upper()))
        lookslikeaudio_func = lambda f: (True if (not nonecodec_func(f.get("acodec")) and nonecodec_func(f.get("vcodec"))) else any(k in label_func(f).lower() for k in ["audio", "aac", "opus", "mp3", "flac", "音频", "音质"]))
        videosortkey_func = lambda f: (lambda h, w, fps, high_br, hdr_dolby, bitrate: (h, w, fps, 1 if bitrate else 0, bitrate or 0, high_br, hdr_dolby, vcodecrank_func(f.get("vcodec"))))(*infervideofields_func(f), (f.get("vbr") or f.get("tbr")))
        audiosortkey_func = lambda f: (lambda bitrate: (1 if bitrate else 0, bitrate, f.get("asr") or 0, f.get("audio_channels") or 0, acodecrank_func(f.get("acodec"))))(f.get("abr") or f.get("tbr") or 0)
        videos = [f for f in formats if lookslikevideo_func(f)] or formats; audios = [f for f in formats if lookslikeaudio_func(f)]
        if include_muxed_in_audio: audios += [f for f in formats if (not nonecodec_func(f.get("acodec"))) and (not nonecodec_func(f.get("vcodec")))]
        videos.sort(key=videosortkey_func, reverse=True); audios.sort(key=audiosortkey_func, reverse=True)
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None) -> list[VideoInfo]:
        ...
        try:
            ...
            ext = 'mkv' if platformfromurl(url) in {'facebook'} else (guess_video_ext_result['ext'] if guess_video_ext_result['ext'] and guess_video_ext_result['ext'] != 'NULL' else video_info['ext'])
            video_info.update(dict(title=video_title, save_path=os.path.join(self.work_dir, self.source, f'{video_title}.{ext}'), ext=ext, guess_video_ext_result=guess_video_ext_result, identifier=video_title))
