'''
Function:
    Implementation WoofVideoClient: https://dl.woof.monster/
Author:
    Zhenchao Jin
...
'''WoofVideoClient'''
class WoofVideoClient(BaseVideoClient):
    source = 'WoofVideoClient'
    def __init__(self, **kwargs):
        super(WoofVideoClient, self).__init__(**kwargs)
        self.default_parse_headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36", "Accept": "application/json", "Content-Type": "application/json", "Origin": "https://api.dl.woof.monster"}
        self.default_download_headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36", "Origin": "https://api.dl.woof.monster", "Accept": "*/*"}
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            (resp := self.post(f'https://api.dl.woof.monster', headers=headers, json=payload, **request_overrides)).raise_for_status()
            ...
            if (ext := guess_video_ext_result['ext'] if guess_video_ext_result['ext'] and guess_video_ext_result['ext'] != 'NULL' else video_info['ext']) in {'m4s'}: ext = 'mp4'
            video_info.update(dict(title=video_title, file_path=os.path.join(self.work_dir, self.source, f'{video_title}.{ext}'), ext=ext, guess_video_ext_result=guess_video_ext_result, identifier=video_title, cover_url=None)); video_infos.append(video_info)
