'''
Function:
    Implementation of APICXVideoClient: https://apicx.asia/doc/video_parse_api
Author:
    Zhenchao Jin
...
'''APICXVideoClient'''
class APICXVideoClient(BaseVideoClient):
    source = 'APICXVideoClient'
    API_TOKEN = 'charlespikachuSDB4Q1Jqc3BUeWtSc0piLS1ja0g3QQ=='
    def __init__(self, **kwargs):
        super(APICXVideoClient, self).__init__(**kwargs)
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None) -> list[VideoInfo]:
        ...
        try:
            ...
            (resp := self.get("https://apicx.asia/api/video_parse_api", params={"url": url, "token": BaseVideoClient.decrypt_func(APICXVideoClient.API_TOKEN)}, headers=headers, **request_overrides)).raise_for_status()
            ...
            # --download url
            video_info.update(dict(download_url=(download_url := safeextractfromdict(raw_data, ['data', 'url'], None) or safeextractfromdict(raw_data, ['data', 'videos', 0, 'url'], None) or safeextractfromdict(raw_data, ['data', 'video_backup', 0, 'url'], None))))
