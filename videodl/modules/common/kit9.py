...
class KIT9VideoClient(BaseVideoClient):
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None) -> list[VideoInfo]:
        ...
        try:
            ...
            (resp := self.get(f'https://apis.kit9.cn/api/aggregate_videos/api.php?link={url}', headers=headers, **request_overrides)).raise_for_status()
            video_info.update(dict(raw_data=(raw_data := resp2json(resp=resp))))
            # --video title
            video_title = legalizestring(safeextractfromdict(raw_data, ['data', 'video_title'], None) or null_backup_title, replace_null_string=null_backup_title).removesuffix('.')
            # --download url
            video_link = raw_data['data']['video_link']
            if isinstance(video_link, dict): video_link = dict(sorted(video_link.items(), key=lambda kv: (int((re.search(r'(\d{3,4})(?=\s*p\b)', str(kv[0]).lower()) or re.search(r'(\d{3,4})', str(kv[0])) or re.search(r'(.*)', '0')).group(1)) if re.search(r'(\d{3,4})(?=\s*p\b)|(\d{3,4})', str(kv[0]).lower()) else -1, str(kv[0])))); video_info.update(dict(download_url=list(video_link.values())[-1]))
            else: video_info.update(dict(download_url=video_link))
            # --other infos
            ...
            video_info.update(dict(title=video_title, save_path=os.path.join(self.work_dir, self.source, f'{video_title}.{ext}'), ext=ext, guess_video_ext_result=guess_video_ext_result, identifier=video_title, cover_url=safeextractfromdict(raw_data, ['data', 'video_cover'], None))); video_infos.append(video_info)
