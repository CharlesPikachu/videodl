'''
Function:
    Implementation of YouChuangVideoClient: https://apis.uctb.cn/apidata?id=15
Author:
    Zhenchao Jin
...
class YouChuangVideoClient(BaseVideoClient):
    source = 'YouChuangVideoClient'
    def __init__(self, **kwargs):
        super(YouChuangVideoClient, self).__init__(**kwargs)
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            resp = self.get(f'http://apis.uctb.cn/api/videojx?url={url}', headers=headers, **request_overrides)
            ...
            video_title = raw_data['data'].get('desc') or f'{self.source}_null_{date_str}'
            video_title = legalizestring(video_title, replace_null_string=f'{self.source}_null_{date_str}').removesuffix('.')
            # --download url
            download_url = raw_data['data']['playurl']
