...
class VideoFKVideoClient(BaseVideoClient):
    ...
    def _parsewebpage(self, html: str):
        ...
        parseheight_func = lambda label: (int(m.group(1)) if label and (m := re.search(r'(\d{3,4})\s*p', label.lower())) else None)
        parsebitratekbps_func = lambda label: (int(m.group(1)) if label and (m := re.search(r'(\d+)\s*kb/s', label.lower())) else None)
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            else: download_url = raw_data['video_formats'][0]['url']; download_url = self.get('https://downloader.twdown.online/load_url?url=' + download_url[download_url.index('url=')+4:], **request_overrides).text.strip() if 'url=' in download_url else download_url
            audio_download_url = 'NULL' if not raw_data['audio_formats'] else raw_data['audio_formats'][0].get('url')
            if audio_download_url and audio_download_url != 'NULL': audio_download_url = self.get('https://downloader.twdown.online/load_url?url=' + audio_download_url[audio_download_url.index('url=')+4:], **request_overrides).text.strip()
            # --deal with special download urls
            download_url = self._convertspecialdownloadurl(download_url)[0]
