...
class XCVTSVideoClient(BaseVideoClient):
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None) -> list[VideoInfo]:
        ...
        try:
            ...
            (resp := self.get(f'https://api.xcvts.cn/api/video_qsy/juhe?apiKey=cda4d99ecb4983e6c62aa6ca2bd2b933&url={url}', headers=headers, **request_overrides)).raise_for_status()
            video_info.update(dict(raw_data=(raw_data := resp2json(resp=resp))))
            # --video title
            video_title = legalizestring(safeextractfromdict(raw_data, ['data', 'video_info', 'title'], None) or null_backup_title, replace_null_string=null_backup_title).removesuffix('.')
            # --download url
            ...
            video_info.update(dict(title=video_title, save_path=os.path.join(self.work_dir, self.source, f'{video_title}.{ext}'), ext=ext, guess_video_ext_result=guess_video_ext_result, identifier=video_title, cover_url=safeextractfromdict(raw_data, ['data', 'video_info', 'cover'], None))); video_infos.append(video_info)
