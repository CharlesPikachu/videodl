...
class CenguiguiVideoClient(BaseVideoClient):
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None):
        ...
        try:
            ...
            try:
                resp = self.get(f'https://api-v1.cenguigui.cn/api/juhe/video.php?url={url}', headers=headers, **request_overrides)
                resp.raise_for_status()
            except:
                resp = self.get(f'https://api.cenguigui.cn/api/juhe/video.php?url={url}', headers=headers, **request_overrides)
                resp.raise_for_status()
