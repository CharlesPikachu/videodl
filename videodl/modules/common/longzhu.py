...
class LongZhuVideoClient(BaseVideoClient):
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None) -> list[VideoInfo]:
        ...
        try:
            ...
            (resp := self.get(f'http://www.hhlqilongzhu.cn/api/sp_jx/sp.php?url={url}', verify=False, headers=headers, **request_overrides)).raise_for_status()
            ...
            video_info.update(dict(title=video_title, save_path=os.path.join(self.work_dir, self.source, f'{video_title}.{ext}'), ext=ext, guess_video_ext_result=guess_video_ext_result, identifier=video_title, cover_url=safeextractfromdict(raw_data, ['data', 'cover'], None))); video_infos.append(video_info)
