...
import time
import copy
from ..sources.base import BaseVideoClient
...
class BVVideoClient(XiazaitoolVideoClient):
    ...
    def __init__(self, **kwargs):
        ...
        self.default_parse_headers = {
            "accept": "application/json, text/javascript, */*; q=0.01", "accept-encoding": "gzip, deflate, br, zstd", "accept-language": "zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7", "content-type": "application/json", "sec-ch-ua-platform": "\"Windows\"", "priority": "u=1, i", 
            "referer": "https://www.bestvideow.com/xhs", "sec-ch-ua": "\"Google Chrome\";v=\"143\", \"Chromium\";v=\"143\", \"Not A(Brand\";v=\"24\"", "sec-ch-ua-mobile": "?0", "sec-fetch-dest": "empty", "sec-fetch-mode": "cors", "origin": "https://www.bestvideow.com",
            "sec-fetch-site": "same-origin", "timestamg": str(int(time.time() * 1000)), "x-requested-with": "XMLHttpRequest", "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36",
        }
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None) -> list[VideoInfo]:
        ...
        try:
            ...
            (resp := self.post("https://www.bestvideow.com/video/parseVideoUrl", json=self._confidential({"url": url, "platform": self._detectplatform(url)}), headers=headers, **request_overrides)).raise_for_status()
            ...
            for item in raw_data['data']['voideDeatilVoList']:
                if not isinstance(item, dict) or not item.get('url') or not str(item['url']).startswith('http'): continue
